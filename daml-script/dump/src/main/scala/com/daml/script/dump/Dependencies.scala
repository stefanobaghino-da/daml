...
object Dependencies {
  ...
  def fetchPackages(client: LedgerClient, references: List[PackageId])(
      implicit ec: ExecutionContext): Future[Map[PackageId, (ByteString, Ast.Package)]] = {
    def go(todo: List[PackageId], acc: Map[PackageId, (ByteString, Ast.Package)])
  ...
  def writeDar(
      sdkVersion: String,
      file: Path,
      dar: Dar[(PackageId, ByteString, Ast.Package)]): Unit = {
  ...
  def toDar(pkgId: PackageId, pkgs: Map[PackageId, (ByteString, Ast.Package)])
