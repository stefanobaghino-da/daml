...
object TreeUtils {
  ...
  def traverseTree(tree: TransactionTree)(f: (List[Crumb], TreeEvent.Kind) => Unit): Unit = {
    ...
    tree.rootEventIds.map(tree.eventsById(_)).zipWithIndex.foreach {
      case (ev, i) =>
  ...
  private def valueParties(v: Value.Sum): Set[String] = v match {
    ...
    case Sum.Record(value) =>
      value.fields.map(v => valueParties(v.getValue.sum)).foldLeft(Set[String]()) {
        case (x, xs) => x.union(xs)
      }
    case Sum.Variant(value) => valueParties(value.getValue.sum)
    case Sum.ContractId(_) => Set()
    case Sum.List(value) =>
      value.elements.map(v => valueParties(v.sum)).foldLeft(Set[String]()) {
        case (x, xs) => x.union(xs)
      }
    ...
    case Sum.Map(value) =>
      value.entries.map(e => valueParties(e.getValue.sum)).foldLeft(Set[String]()) {
        case (x, xs) => x.union(xs)
      }
