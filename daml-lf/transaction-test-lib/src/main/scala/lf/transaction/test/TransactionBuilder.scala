...
final class TransactionBuilder(pkgTxVersion: Ref.PackageId => TransactionVersion) {
  ...
  private var nodes = HashMap.newBuilder[NodeId, GenNode.WithValue[NodeId, ContractId]]
  ...
  def build(): Tx.Transaction = ids.synchronized {
    ...
    val builtNodes = nodes.result().map { case (nid, node) => nid -> versionN(node) }
    ...
    VersionedTransaction(txVersion, builtNodes, roots.result())
  }
...
//  }
  ...
  def versionContract(contract: ContractInst[Value]): ContractInst[TxValue] =
    ContractInst.map1[Value, TxValue](versionValue(pkgTxVersion(contract.template.packageId)))(
      contract)

  private[this] def versionValue(version: TransactionVersion): Value => TxValue =
    value.Value.VersionedValue(TransactionVersions.assignValueVersion(version), _)

  private[this] def versionN(node: Node): VersionedNode[NodeId, ContractId] = {
    val version = pkgTxVersion(node.templateId.packageId)
    VersionedNode(
      version,
      GenNode.map3(identity[NodeId], identity[ContractId], versionValue(version))(node)
    )
  }
